...
type BookRow = { _id: string; title: string; author?: string; isbn?: string; copies: number; available: number };
type IssueRow = {
  _id: string;
  dueAt?: string;
  status?: string;
...
const emptyBook = { title: "", author: "", isbn: "", copies: 1 };
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={
          <>
            <Button variant="secondary" onClick={() => setIssueOpen(true)}>{t.library.issue}</Button>
            <Button onClick={() => { setForm(emptyBook); setEdit(null); setBookOpen(true); }}>{t.common.add}</Button>
          </>
        }
      />
        ...
        <>
          ...
          <DataTable
            ...
            columns={[
              ...
              { header: t.library.isbn, cell: (row) => row.isbn || "—" },
              { header: t.library.stock, cell: (row) => `${row.available} / ${row.copies}`, align: "right", sortValue: (row) => row.available },
            ]}
            actions={(row) => [
              {
                label: t.common.edit,
                onClick: () => {
                  setEdit(row);
                  setForm({ title: row.title, author: row.author ?? "", isbn: row.isbn ?? "", copies: row.copies });
                  setBookOpen(true);
                },
              },
        ...
        <>
          ...
          <DataTable
            ...
            columns={[
              ...
              { header: t.common.student, cell: (row) => `${row.studentId?.name ?? "—"} ${row.studentId?.studentId ?? ""}`.trim() },
              { header: t.library.due, cell: (row) => (row.dueAt ? new Date(row.dueAt).toLocaleDateString() : "—") },
              {
      ...
      <Dialog open={bookOpen} title={edit ? t.common.edit : t.common.add} onClose={() => { setBookOpen(false); setEdit(null); }}>
        <form
          className="space-y-3"
          onSubmit={async (e) => {
            e.preventDefault();
        ...
        >
          <Field label={t.library.book}><Input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} required /></Field>
          <Field label={t.library.author}><Input value={form.author} onChange={(e) => setForm({ ...form, author: e.target.value })} /></Field>
          <Field label={t.library.isbn}><Input value={form.isbn} onChange={(e) => setForm({ ...form, isbn: e.target.value })} /></Field>
          <Field label={t.library.copies}><Input type="number" min={1} value={form.copies} onChange={(e) => setForm({ ...form, copies: Number(e.target.value) })} /></Field>
          <div className="flex justify-end gap-2">
            <Button type="button" variant="secondary" onClick={() => { setBookOpen(false); setEdit(null); }}>{t.common.cancel}</Button>
        ...
        >
          <Field label={t.library.book}>
            <Select value={issueForm.bookId} onChange={(e) => setIssueForm({ ...issueForm, bookId: e.target.value })}>
              ...
              {rows.filter((row) => row.available > 0).map((row) => (
          ...
          <Field label={t.common.student}>
            <Select value={issueForm.studentId} onChange={(e) => setIssueForm({ ...issueForm, studentId: e.target.value })}>
              ...
              {studentList.map((row) => (
                <option key={row._id} value={row._id}>{row.name} · {row.studentId}</option>
              ))}
            </Select>
          </Field>
          <Field label={t.library.due}>
            <Input type="date" value={issueForm.dueAt} onChange={(e) => setIssueForm({ ...issueForm, dueAt: e.target.value })} />
          </Field>
          <div className="flex justify-end gap-2">
            <Button type="button" variant="secondary" onClick={() => setIssueOpen(false)}>{t.common.cancel}</Button>
            <Button disabled={!issueForm.bookId || !issueForm.studentId || !issueForm.dueAt} type="submit">{t.library.issue}</Button>
          </div>
        </form>
      </Dialog>
      <ConfirmDialog
